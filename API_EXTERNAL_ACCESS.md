...
1. **API密钥管理（前端界面）**
   - 管理员登录后可在Web界面管理API密钥
   - 生成API密钥
   - 查看API密钥列表
   - 删除API密钥
   - 复制API密钥
   - 查看密钥使用状态（创建时间、过期时间、最后使用时间等）
...
1. **登录系统**：使用管理员账号登录Cloud Mail

2. **导航到API密钥管理**：
   - 在左侧菜单中点击"管理"分组下的"API密钥"
   - 或直接访问路径：`/api-key`
...
1. 点击页面右上角的"+"图标
2. 在弹出的对话框中填写：
   - **密钥名称**：为密钥起一个易于识别的名称（如"外部系统A"、"Webhook集成"等）
   - **有效天数**：设置密钥有效期，0表示永不过期
3. 点击"生成"按钮
4. 系统会生成一个新的API密钥，并自动复制到剪贴板
